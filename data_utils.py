...
def ped_op_is_bounded(segment, err):
    err_dict = {}
    if len(segment) <= 2:
        # print('segment error', 0.0)
        return True
...
def ped_op_is_bounded_with_err(segment, err):
    err_dict = {}
    if len(segment) <= 2:
        # print('segment error', 0.0)
        return 0.0, True
...
def sed_op_is_bounded(segment, err):
    if len(segment) <= 2:
        # print('segment error', 0.0)
        return True
    else:
...
def sed_op_is_bounded_with_err(segment, err):
    if len(segment) <= 2:
        # print('segment error', 0.0)
        return 0.0, True
    else:
...
def _draw(ori_traj, sim_traj, label='sed'):
...
def draw(ori_traj, sim_traj, label='sed'):
    error, error_points, error_left, error_right, error_syn = draw_error(ori_traj, sim_traj, label)
    pdf = PdfPages('vis_marlts_geolife_sed.pdf')
    plt.figure(figsize=(10.5/2,6.8/2)) 
    plt.plot(np.array(ori_traj)[:,0],np.array(ori_traj)[:,1],color="blue", linewidth=0.5, label='raw traj')
    plt.scatter(np.array(sim_traj)[:,0],np.array(sim_traj)[:,1],color="red", s=20, label='points in simplified traj')
    #plt.scatter(error_points[0],error_points[1],color="black", s=30, marker='s', label='maximal error point')
    #plt.plot([error_points[0],error_syn[0]],[error_points[1],error_syn[1]], '--', color="black", label='SED')
    #plt.plot([error_left[0],error_right[0]],[error_left[1],error_right[1]], color="green", linewidth=2, label='error segement')
    #plt.title('simplification length: '+str(len(sim_traj)))
    plt.legend(loc='best')
    #plt.show()
    pdf.savefig()
    pdf.close()
